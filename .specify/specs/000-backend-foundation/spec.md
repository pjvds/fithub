...
- **FR-21:** System MUST expose `GET /api/sync/history?limit=<n>&cursor=<cursor>` (authenticated) returning a paginated list of sync job records for the requesting user. Each record MUST include: `id`, `platform`, `status` (`success` | `partial` | `failed`), `started_at`, `completed_at`, `activities_synced` (integer), and `error` (string | null). Response MUST include `next_cursor` for subsequent pages. Returns an empty `jobs: []` array when no sync history exists.
- **FR-22:** System MUST expose `GET /api/user/profile` (authenticated) returning the requesting user's profile: `email`, `created_at` (ISO 8601), and `connections` (array of active platform connections per the `Connection` type — `platform`, `connected_at`, `last_synced_at`, `last_error`). A 404 is returned only if the user record does not exist.
