...
Seven phases sequenced (see Implementation Breakdown below for the canonical phase numbering): Phase 1 establishes SST project + D1 schema + outbox infrastructure + JWT validation middleware; Phase 2 builds OAuth Token Vault + adapter interface; Phase 3 adds platform connections (OAuth initiate/callback/list/disconnect) and concrete Zwift + Strava adapters + token refresh; Phase 4 adds the Sync Orchestrator (Zwift cron poller, Strava webhooks, sync worker, retry queue, rate-limit budget); Phase 5 builds the Deduplication Engine; Phase 6 adds the Push Notification Service (APNs/FCM — DEFERRED to mobile v2+); Phase 7 hardens (observability, alerts, GDPR, audit log archival, load tests, security review). FitHub user authentication itself (the OpenAuth `auth` Worker for Apple/Google/email-magic-link sign-in) is specified in feature `001-user-authentication` — it is a hard dependency of this foundation but specified and tracked separately.
